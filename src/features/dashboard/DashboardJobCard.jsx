...
const DashboardJobCard = ({ title, company, location, type, duration, paid, description, id }) => {
    return (
        <div className="bg-white p-4 sm:p-6 rounded-lg shadow-md flex flex-col h-full">
            <h3 className="text-base sm:text-lg font-semibold text-blue-900 mb-1 sm:mb-2">{title}</h3>
            <p className="text-grey-600 text-xs sm:text-sm mb-2 sm:mb-3">{company}</p>
            <p className="text-grey-600 -900 text-xs sm:text-sm mb-3 sm:mb-4 line-clamp-3 flex-grow">{description}</p>
            ...
            <div className="flex flex-wrap gap-1 sm:gap-2 mb-3 sm:mb-4 mt-auto">
                ...
                {duration && (
                    <span className="bg-grey-100 text-grey-600 text-[10px] sm:text-xs font-medium px-2 py-0.5 rounded-full">
                        {duration}
                    </span>
                )}
                {paid && (
