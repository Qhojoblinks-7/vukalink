...
import { signIn } from '../../services/auth'; // Ensure this path is correct
...
// LoginForm now accepts a prop 'onToggleForm' from its parent (AuthPage)
const LoginForm = ({ onToggleForm }) => {
  const { signInWithGoogle } = useAuth();
  const [serverError, setServerError] = useState('');
  const [loading, setLoading] = useState(false);
  const [showManualLoginFields, setShowManualLoginFields] = useState(false);
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      if (signInWithGoogle) {
        ...
        console.log("Google sign-in initiated. AuthContext will handle redirect.");
      } else {
        setServerError("Google sign-in not configured.");
        console.error("signInWithGoogle function is not available.");
      }
    } catch (authError) {
      setServerError(authError.message || "Failed to sign in with Google.");
  ...
  return (
    ...
    <div className="bg-white rounded-lg shadow-xl p-8 max-w-4xl w-full mx-auto flex flex-col md:flex-row gap-8 md:gap-12 items-center">
      ...
      {/* Left Column: Branding and Google Sign-in */}
      <div className="flex flex-col items-center justify-center text-center p-4 md:p-8 md:w-1/2 space-y-4">
        ...
        <h2 className="font-heading text-gray-800 text-2xl font-semibold mt-4">Welcome Back!</h2>
        <p className="font-body text-gray-600 text-lg">Your journey starts here</p>
      ...
      <div className="flex flex-col p-4 md:p-8 md:w-1/2">
        {/* "or sign in manually" Separator - acts as a trigger on mobile */}
        <div
          className="flex items-center my-4 cursor-pointer hover:opacity-80 transition-opacity"
          onClick={() => setShowManualLoginFields(!showManualLoginFields)}
        >
          ...
          <span className="px-3 text-gray-500 text-sm font-body flex items-center">
            ...
            <svg
              className={`ml-2 h-4 w-4 text-gray-500 transition-transform md:hidden ${showManualLoginFields ? 'rotate-180' : ''}`}
        ...
        {/* Manual Login Form section - hidden on mobile by default, shown on click, always shown on md+ */}
        <div className={`${showManualLoginFields ? 'block' : 'hidden'} md:block`}>
          {/* Formik Wrapper for Email/Password Login */}
          <Formik
            ...
            {({ isSubmitting, errors, touched, getFieldProps }) => (
              <Form className="space-y-4">
                ...
                <div className="flex justify-between items-center text-sm font-body">
                  <label htmlFor="remember-me" className="flex items-center text-gray-600 cursor-pointer">
                    <Field
                      type="checkbox"
                      id="remember-me"
                      name="rememberMe"
          ...
          <div className="bg-blue-50 border border-blue-200 text-blue-800 p-3 rounded-md flex items-start space-x-2 text-sm font-body mt-4">
            ...
            <p>
              For the best experience, we recommend signing in with your Google account for seamless access and security.
            </p>
          </div>
