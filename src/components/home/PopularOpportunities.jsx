...
const OpportunityCard = ({ title, company, location, type, tags }) => (
  <div className="bg-white p-6 rounded-lg shadow-md">
    ...
    <p className="text-grey-500 mb-3">{company} · {location}</p>
    <div className="flex flex-wrap gap-2 mb-4">
      <span className="bg-blue-50 text-blue-900 text-xs font-semibold px-2.5 py-0.5 rounded-full">{type}</span>
      {tags.map((tag, index) => (
        <span key={index} className="bg-gray-200 text-gray-700 text-xs font-medium px-2.5 py-0.5 rounded-full">{tag}</span>
      ))}
    </div>
